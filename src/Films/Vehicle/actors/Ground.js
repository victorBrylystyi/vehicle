...
class Ground extends GObject{
    constructor(phWorld,dim,initialPos){
        ...
        this.physicMeshMatrtial =  new THREE.MeshStandardMaterial({
    ...
    createHeightFieldShape(dim){
        ...
        for (let i = 0; i < dim.sizeX; i++) {
          ...
          for (let j = 0; j < dim.sizeY; j++) {
              let height = Math.sin(i / dim.sizeX * Math.PI * 8) * Math.sin(j / dim.sizeY * Math.PI * 8) * dim.amplitude + heightPlane;
